...
class Game(Widget):
    ...
    def update(self, dt): # the big update function that updates everything that needs to be updated at each discrete time t when reaching a new state (getting new signals from the sensors)
        ...
        orientation = Vector(*self.car.velocity).angle((xx,yy))/180. # direction of the with respect to the goal (if the car is heading perfectly towards the goal, then orientation = 0)
